...
void FunStartStopCheck(manager *pFlowPcb);
...
const partab_attr_type ParTabAttr[] = { {0x0000, PAR_VERSION_ID, PAR_VERSION_ID, PAR_VERSION_ID, "VERSIONID"},
    ...
    {0x0000, -500, 500, 0, "Object Calib"}, //34
    ...
    if (ParTab[PAR_POS_Run_Stop_Command].uVal != LastPAR_POS_Run_Stop_Command) {
        ...
        if (ParTab[PAR_POS_Run_Stop_Command].uVal == 1) {
            ...
            if (Procedure.type == Melt || Procedure.type == Liquid) {
                ...
                //	         ParTab[PAR_POS_FUNCTION_SET].uVal  == 1  ?  ( Procedure.SteriTimes = 2 ) :  ( Procedure.SteriTimes = 1 );
                pFlowPcb->flow_state =  WORK_PROGRAM_FLOW_STATE_HEAT1 ;
            }
...
{
    Temperature.P = RunFlow.StateParameter.DryStartFlag ? ParTab[PAR_POS_DRY_P ].uVal : ParTab[PAR_POS_P_SET ].uVal;
    Temperature.I = RunFlow.StateParameter.DryStartFlag ? ParTab[PAR_POS_DRY_I ].uVal : ParTab[PAR_POS_I_SET ].uVal;
    Temperature.d = RunFlow.StateParameter.DryStartFlag ? ParTab[PAR_POS_DRY_D ].uVal : ParTab[PAR_POS_D_SET ].uVal;
    Temperature.AR = RunFlow.StateParameter.DryStartFlag ? ParTab[PAR_POS_DRY_AR ].uVal : ParTab[PAR_POS_AR_SET ].uVal;
    Temperature.T = RunFlow.StateParameter.DryStartFlag ? ParTab[PAR_POS_DRY_T ].uVal : ParTab[PAR_POS_T_SET ].uVal;
}
...
void ParaTask(void *argument)
{
    ...
    for (;;) {
        ...
        osThreadFlagsWait(TP_BIT_0, TP_BIT_0, osWaitForever);
        FunStartStopCheck(&RunFlow);
        Para_PID_Update();
        ParSaveCheck();
